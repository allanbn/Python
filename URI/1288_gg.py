...
def sol(i, pos, knapsack, pp, N, T, PODER=0, PESO=1):
    ...
    knapsack[i][pos] = sol(i+1, pos, knapsack, pp, N, T)
    if pos + pp[i][PESO] <= T:
        knapsack[i][pos] = max(knapsack[i][pos], pp[i][PODER] + sol(i + 1, pos + pp[i][PESO], knapsack, pp, N, T))
